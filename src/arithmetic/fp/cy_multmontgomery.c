...
/* 																			      		*/
/* The code is intended to enable 32, 64 and 128 bits multiplier in later versions		*/
...
/* Conditional substraction, forked from nn_cnd_sub of libECC for generic size (including vectors)*/
...
/* TODO: make implementation fully interoperable, there are some implicit low and high part calls*/
        ...
        const cy_word_t p[], const cy_word_t n0, size_t size)
{
    ...
    for(i=0; i<size; i++)
    {
        ...
        ADDCTO_MWORD(r[size], tu64_carry, l_carry);// r[size]+=carry;//devil carry here
        IO_print_montgo_array("\n# r loop1: ", r, 3);

       temp[0]=tu64_carry;// temp=(r[size]<carry?1:0);//propagated by addtom in tu64_carry
       printf("\n # temp=%d", temp[1]);
