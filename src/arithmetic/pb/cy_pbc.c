...
static cy_error_t cy_line_dbl(cy_fp6_t line, POINTonE2 *T, const POINTonE2 *Q)
{
...
static cy_error_t cy_line_by_Px2(cy_fp6_t line, const POINTonE1_affine *Px2)
{
	error=CY_OK;
//    mul_fp(line[1][0], line[1][0], Px2->X);   /* "b01" *= -2*P->X */
	cy_fp_mul( line[1][0], Px2->X, line[1][0]);

 //   mul_fp(line[1][1], line[1][1], Px2->X);
	cy_fp_mul( line[1][1], Px2->X, line[1][1]);

//    mul_fp(line[2][0], line[2][0], Px2->Y);   /* "b11" *= 2*P->Y */
	cy_fp_mul( line[2][0], Px2->Y, line[2][0]);

//    mul_fp(line[2][1], line[2][1], Px2->Y);
    cy_fp_mul( line[2][1], Px2->Y, line[2][1]);

    return error;

}

static cy_error_t cy_start_dbl_n(vec384fp12 ret, POINTonE2 T[],
...
static cy_error_t cy_add_n_dbl_n(vec384fp12 ret, POINTonE2 T[],
                                        const POINTonE2_affine Q[],
                                        ...
                                        size_t n, size_t k)
{
    ...
    for (i = 0; i < n; i++) {
        cy_line_add(line, T+i, T+i, Q+i);  line_by_Px2(line, Px2+i);
    ...
    while (k--) {
        ...
        for (i = 0; i < n; i++) {
            cy_line_dbl(line, T+i, T+i);   line_by_Px2(line, Px2+i);
            mul_by_xy00z0_fp12(ret, ret, line);
        }
    }
}
