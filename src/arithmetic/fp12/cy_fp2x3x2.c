...
static inline cy_error_t cy_mul_by_u_plus_1_fp2( cy_fp2_t *a, cy_fp2_t *ret)
{
  cy_error_t error = CY_KO;
 ...
 * Fp6 extension
 */
static inline cy_error_t cy_fp6_mul(cy_fp6_t ret, const cy_fp6_t a, const cy_fp6_t b)
{
 cy_error_t error;
...
static cy_error_t sqr_fp6( const cy_fp6_t a, cy_fp6_t ret)
{
...
static cy_error_t add_fp6(const cy_fp6_t a, const cy_fp6_t b, cy_fp6_t ret)
{
	cy_error_t error;
...
static cy_error_t sub_fp6(const cy_fp6_t a, const cy_fp6_t b, cy_fp6_t ret)
{
 cy_error_t error;
...
static cy_error_t neg_fp6( const cy_fp6_t a, cy_fp6_t ret)
{
  cy_error_t error;
//	neg_fp2(ret[0], a[0]);
...
cy_error_t cy_fp2x3x2_mul(cy_fp12_t *a, const cy_fp12_t *b, const cy_fp12_t *ret)
{
  cy_error_t error = CY_KO;
...
//   sub_fp6(ret[1], ret[1], t1);
 ...
 //  add_fp2(ret[0][0], t0[0], t1[2]);
  cy_fp6_add(&t0[0], &t1[2], &ret[0][0]);

//   add_fp2(ret[0][1], t0[1], t1[0]);
  cy_fp6_add(&t0[1], &t1[0], &ret[0][1]);

//   add_fp2(ret[0][2], t0[2], t1[1]);
  cy_fp6_add(&t0[2], &t1[1], &ret[0][2]);
...
static inline cy_error_t mul_by_0y0_fp6(cy_fp6_t *ret, const cy_fp6_t *a,
                                  const cy_fp2_t *b)
{
 // vec384x t;
cy_fp6_t t;
...
//todo
static cy_error_t mul_by_xy00z0_fp12(cy_fp12_t *ret, const cy_fp12_t *a,
...
//  add_fp2(ret[0][1], t0[1], t1[0]);
  cy_fp6_add(t1[0], t0[1], ret[0][1]);

//  add_fp2(ret[0][2], t0[2], t1[1]);
  cy_fp6_add(t1[1], t0[2], ret[0][2]);
...
static cy_error_cy_fp2x3x2_sqr( const cy_fp12_t *a, cy_fp12_t *ret)
{
	cy_error_t error;
//  vec384fp6 t0, t1;
  cy_fp6_t t0, t1;
...
//  sub_fp2(ret[0][2], ret[0][2], t1[1]);
  cy_fp2_sub(ret[0][2], t1[1], ret[0][2]);
...
static cy_error_t cy_fp6_inv(const cy_pf6_t a, cy_fp6_t *ret)
{
	cy_error_t error;
...
static cy_error_t cy_fp12_inv(const cy_fp12_t *a, cy_fp12_t *ret)
{
	cy_error_t error;
	cy_fp6_t t0, t1;
...
//  inverse_fp6(t1, t0);
 ...
 // mul_fp6(ret[0], a[0], t1);
  cy_fp2_mul(&a[0],  &t1, &ret[0]);

//  mul_fp6(ret[1], a[1], t1);
  cy_fp2_mul(&a[1],  &t1, &ret[1]);
...
static cy_error_t cy_fp4_sqr( const cy_fp4_t *a0, const cy_fp4_t *a1, cy_fp4_t *ret)
{
	cy_error_t error;
  cy_fp2_t t0, t1;
...
static cy_error_t cy_fp12_cyclotomic_sqr(const cy_fp12_t *a, cy_fp12_t *ret)
{
	cy_error_t error;
...
//  add_fp2(ret[1][1], t0[1], a[1][1]);
  cy_fp2_add( &t0[1][1], &a[1][1], &ret[1][1]);
