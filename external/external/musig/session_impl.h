...
static const unsigned char secp256k1_musig_secnonce_magic[4] = { 0x22, 0x0e,
                                                                 0xdc, 0xf1 };
...
{
  int is_zero;
  ARG_CHECK(secp256k1_memcmp_var(&secnonce->data[0],
                                 secp256k1_musig_secnonce_magic, 4) == 0);
 ...
 */
static void secp256k1_musig_secnonce_invalidate(
    const secp256k1_context *ctx, secp256k1_musig_secnonce *secnonce, int flag)
{
  ...
  secp256k1_declassify(ctx, secnonce->data,
                       sizeof(secp256k1_musig_secnonce_magic));
}

static const unsigned char secp256k1_musig_pubnonce_magic[4] = { 0xf5, 0x7a,
                                                                 0x3d, 0xa0 };
                                         ...
                                         const secp256k1_musig_pubnonce *nonce)
{
  ...
  ARG_CHECK(secp256k1_memcmp_var(&nonce->data[0],
                                 secp256k1_musig_pubnonce_magic, 4) == 0);
  for (i = 0; i < 2; i++) {
...
{
  return secp256k1_musig_pubnonce_load(ctx, ge,
 ...
 */
static void
secp256k1_musig_session_save(secp256k1_musig_session *session,
...
static int
secp256k1_musig_session_load(const secp256k1_context *ctx,
                             secp256k1_musig_session_internal *session_i,
                             const secp256k1_musig_session *session)
{
  const unsigned char *ptr = session->data;

  ARG_CHECK(secp256k1_memcmp_var(ptr, secp256k1_musig_session_cache_magic, 4) ==
            0);
  ptr += 4;
...
static const unsigned char secp256k1_musig_partial_sig_magic[4] = { 0xeb, 0xfb,
                                                                    0x1a,
                                                                    0x32 };
...
static int
secp256k1_musig_partial_sig_load(const secp256k1_context *ctx,
                                 secp256k1_scalar *s,
                                 const secp256k1_musig_partial_sig *sig)
{
  int overflow;

  ARG_CHECK(secp256k1_memcmp_var(&sig->data[0],
                                 secp256k1_musig_partial_sig_magic, 4) == 0);
...
int secp256k1_musig_aggnonce_serialize(const secp256k1_context *ctx,
                                       ...
                                       const secp256k1_musig_aggnonce *nonce)
{
  return secp256k1_musig_pubnonce_serialize(ctx, out66,
                                            (secp256k1_musig_pubnonce *)nonce);
}
...
int secp256k1_musig_aggnonce_parse(const secp256k1_context *ctx,
                                   ...
                                   const unsigned char *in66)
{
  return secp256k1_musig_pubnonce_parse(ctx, (secp256k1_musig_pubnonce *)nonce,
                                        in66);
}

int secp256k1_musig_partial_sig_serialize(
    const secp256k1_context *ctx, unsigned char *out32,
    const secp256k1_musig_partial_sig *sig)
{
  VERIFY_CHECK(ctx != NULL);
...
static int secp256k1_xonly_ge_serialize(unsigned char *output32,
                                        secp256k1_ge *ge)
{
                                           ...
                                           const unsigned char *extra_input32)
{
  ...
  secp256k1_sha256_initialize_tagged(&sha, (unsigned char *)"MuSig/nonce",
                                     sizeof("MuSig/nonce") - 1);
                              ...
                              const unsigned char *extra_input32)
{
  ...
  secp256k1_nonce_function_musig(k, session_id32, msg32, seckey, pk_ser_ptr,
                                 extra_input32);
...
static int secp256k1_musig_sum_nonces(
    const secp256k1_context *ctx, secp256k1_gej *summed_nonces,
    const secp256k1_musig_pubnonce *const *pubnonces, size_t n_pubnonces)
{
  size_t i;
  int j;
  ...
  for (i = 0; i < n_pubnonces; i++) {
    ...
    for (j = 0; j < 2; j++) {
      secp256k1_gej_add_ge_var(&summed_nonces[j], &summed_nonces[j],
                               &nonce_pt[j], NULL);
    }
  }
  return 1;
...
static int secp256k1_musig_compute_noncehash(unsigned char *noncehash,
                                             ...
                                             const unsigned char *msg)
{
  ...
  secp256k1_sha256_initialize_tagged(&sha, (unsigned char *)"MuSig/noncecoef",
                                     sizeof("MuSig/noncecoef") - 1);
  for (i = 0; i < 2; i++) {
    size_t size;
...
static int secp256k1_musig_nonce_process_internal(int *fin_nonce_parity,
...
int secp256k1_musig_nonce_process(
    const secp256k1_context *ctx, secp256k1_musig_session *session,
    const secp256k1_musig_aggnonce *aggnonce, const unsigned char *msg32,
    const secp256k1_musig_keyagg_cache *keyagg_cache,
    const secp256k1_pubkey *adaptor)
{
  ...
  if (adaptor != NULL) {
    ...
    secp256k1_gej_add_ge_var(&aggnonce_ptj[0], &aggnonce_ptj[0], &adaptorp,
                             NULL);
  }
  if (!secp256k1_musig_nonce_process_internal(&session_i.fin_nonce_parity,
                                              fin_nonce, &session_i.noncecoef,
                                              aggnonce_ptj, agg_pk32, msg32)) {
    return 0;
  }

  secp256k1_schnorrsig_challenge(&session_i.challenge, fin_nonce, msg32, 32,
                                 agg_pk32);
...
void secp256k1_musig_partial_sign_clear(secp256k1_scalar *sk,
                                        secp256k1_scalar *k)
{
  secp256k1_scalar_clear(sk);
...
int secp256k1_musig_partial_sign(
    const secp256k1_context *ctx, secp256k1_musig_partial_sig *partial_sig,
    secp256k1_musig_secnonce *secnonce, const secp256k1_keypair *keypair,
    const secp256k1_musig_keyagg_cache *keyagg_cache,
    const secp256k1_musig_session *session)
{
  secp256k1_scalar sk;
    ...
    const secp256k1_musig_session *session)
{
  ...
  /* TODO: use multiexp to compute -s*G + e*mu*pubkey + aggnonce[0] +
   * b*aggnonce[1] */
...
int secp256k1_musig_partial_sig_agg(
    ...
    const secp256k1_musig_session *session,
    const secp256k1_musig_partial_sig *const *partial_sigs, size_t n_sigs)
{
  size_t i;
  secp256k1_musig_session_internal session_i;
