...
static const unsigned char secp256k1_musig_keyagg_cache_magic[4] = { 0xf4, 0xad,
                                                                     0xbb,
                                                                     0xdf };
 ...
 * - 32 byte tweak
 */
/* Requires that cache_i->pk is not infinity and cache_i->second_pk_x to be
 * normalized. */
static void
secp256k1_keyagg_cache_save(secp256k1_musig_keyagg_cache *cache,
...
static int
secp256k1_keyagg_cache_load(const secp256k1_context *ctx,
                            secp256k1_keyagg_cache_internal *cache_i,
                            const secp256k1_musig_keyagg_cache *cache)
{
  const unsigned char *ptr = cache->data;
  ARG_CHECK(secp256k1_memcmp_var(ptr, secp256k1_musig_keyagg_cache_magic, 4) ==
            0);
  ptr += 4;
 ...
 * applying SHA256 to SHA256("KeyAgg list")||SHA256("KeyAgg list"). */
...
static int secp256k1_musig_compute_pk_hash(
    const secp256k1_context *ctx, unsigned char *pk_hash,
    const secp256k1_xonly_pubkey *const *pk, size_t np)
{
...
/* Initializes SHA256 with fixed midstate. This midstate was computed by
 * applying SHA256 to SHA256("KeyAgg coefficient")||SHA256("KeyAgg
 * coefficient"). */
...
static void secp256k1_musig_keyaggcoef_internal(secp256k1_scalar *r,
                                                const unsigned char *pk_hash,
                                                const secp256k1_fe *x,
                                                const secp256k1_fe *second_pk_x)
{
  secp256k1_sha256 sha;
...
static void
secp256k1_musig_keyaggcoef(secp256k1_scalar *r,
                           const secp256k1_keyagg_cache_internal *cache_i,
                           secp256k1_fe *x)
{
  secp256k1_musig_keyaggcoef_internal(r, cache_i->pk_hash, x,
                                      &cache_i->second_pk_x);
}
...
/* Callback for batch EC multiplication to compute keyaggcoef_0*P0 +
 * keyaggcoef_1*P1 + ...  */
static int secp256k1_musig_pubkey_agg_callback(secp256k1_scalar *sc,
                                               secp256k1_ge *pt, size_t idx,
                                               void *data)
{
  secp256k1_musig_pubkey_agg_ecmult_data *ctx =
      (secp256k1_musig_pubkey_agg_ecmult_data *)data;
  int ret;
  ...
  VERIFY_CHECK(ret);
  secp256k1_musig_keyaggcoef_internal(sc, ctx->pk_hash, &pt->x,
                                      &ctx->second_pk_x);
  return 1;
}
                               ...
                               size_t n_pubkeys)
{
  ...
  for (i = 1; i < n_pubkeys; i++) {
    if (secp256k1_memcmp_var(pubkeys[0], pubkeys[i], sizeof(*pubkeys[0])) !=
        0) {
      secp256k1_ge pt;
  ...
  if (!secp256k1_musig_compute_pk_hash(ctx, ecmult_data.pk_hash, pubkeys,
                                       n_pubkeys)) {
    return 0;
  }
  /* TODO: actually use optimized ecmult_multi algorithms by providing a
   * scratch space */
  if (!secp256k1_ecmult_multi_var(&ctx->error_callback, NULL, &pkj, NULL,
                                  secp256k1_musig_pubkey_agg_callback,
                                  (void *)&ecmult_data, n_pubkeys)) {
...
static int secp256k1_musig_pubkey_tweak_add_internal(
    const secp256k1_context *ctx, secp256k1_pubkey *output_pubkey,
    secp256k1_musig_keyagg_cache *keyagg_cache, const unsigned char *tweak32,
    int xonly)
{
  secp256k1_keyagg_cache_internal cache_i;
  int overflow = 0;
...
int secp256k1_musig_pubkey_ec_tweak_add(
    ...
    secp256k1_musig_keyagg_cache *keyagg_cache, const unsigned char *tweak32)
{
  return secp256k1_musig_pubkey_tweak_add_internal(ctx, output_pubkey,
                                                   keyagg_cache, tweak32, 0);
}
